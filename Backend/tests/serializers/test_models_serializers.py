...
class TestItemProductServiceSerializers:
    ...
    def test_service_valid_duration_passes(self):
        seller = UserFactory(roles=["Seller"])
...
class TestItemCategoryOrderSerializers:
    def test_item_category_serializer(self, user, product_factory):
        prod = product_factory(seller=UserFactory(roles=["Seller"]))
    ...
    def test_order_item_serializer_total_price(self, product_factory):
        seller = UserFactory(roles=["Seller"])
        prod = product_factory(seller=seller, price_cents=250)
        buyer = UserFactory(roles=["Buyer"])
    ...
    def test_order_serializer_nested_create(self, product_factory):
        seller = UserFactory(roles=["Seller"])
        buyer = UserFactory(roles=["Buyer"])
        prod = product_factory(seller=seller, price_cents=100)
        payload = {
            'user': buyer.id,
    ...
    def test_order_serializer_calculates_total(self, product_factory):
        seller = UserFactory(roles=["Seller"])
        buyer = UserFactory(roles=["Buyer"])
        product = product_factory(price_cents=1_000, seller=seller)
        payload = {
            "user": buyer.id,
            "order_items": [
